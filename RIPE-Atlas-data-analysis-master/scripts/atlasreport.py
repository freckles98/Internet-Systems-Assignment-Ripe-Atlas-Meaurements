            ...
            if not 'dnsReport' in results:
                ...
                try:
                    ...
                    pipe = subprocess.Popen(
                        [script, "-d", "%d" % args.detail], stdin=subprocess.PIPE)
...
parser = argparse.ArgumentParser(
    description='Produce report on Atlas measurements.')
parser.add_argument('-v', '--verbosity', action="count", default=0)
parser.add_argument(
    '-d', '--detail', help='level of detail to report', type=int, default=1)
parser.add_argument(
    '-m', '--maxdest', help='max number of destination IPs to report on individually; when exceeded produce aggreate report  ', type=int, default=3)
...
if corrupted:
    print >>sys.stderr, "warning: %d lines out of %d with incomplete JSON data (probe_id,msm_id or fw missing)" % (
        corrupted, totalLines)
    del results['corrupt']

oldformat = results.get('oldformat')
if oldformat:
    print >>sys.stderr, "warning: %d lines out of %d using deprected data format (probe firmware < 4000)" % (
        oldformat, totalLines)
